...
public interface WishListRepository extends JpaRepository<WishList, Long> {
    ...
    @Query("select w from User u join u.friends f join f.wishLists w where u.id=?1 order by w.created DESC")
    List<WishList> getAllFriendWishes(Long userId);

    @Query("select aw from WishList aw order by aw.created DESC")
